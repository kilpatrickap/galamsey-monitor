...
def process_single_gee_frame(aoi_coords, period1_start, period1_end,
    ...
    def calculate_ndvi(image): return image.normalizedDifference(['B8', 'B4']).rename('NDVI')
    ...
    thumb_url = None
    try:
        thumb_url = final_image_viz.getThumbURL({'region': aoi.getInfo()['coordinates'], 'dimensions': thumb_size_val, 'format': 'png'})
        ...
        if "No valid pixels" in str(thumb_e) or "Image has no bands" in str(thumb_e):
            ...
            try:
                thumb_url = background_layer.getThumbURL({'region': aoi.getInfo()['coordinates'], 'dimensions': thumb_size_val, 'format': 'png'})
...
class GEEWorker(QObject):
    def __init__(self, aoi_coords, start1, end1, start2, end2, threshold, thumb_size=512, project_id='galamsey-monitor'):
        super().__init__()
        self.signals = WorkerSignals(); self.aoi_coords = aoi_coords; self.start1 = start1; self.end1 = end1
        self.start2 = start2; self.end2 = end2; self.threshold = threshold; self.thumb_size = thumb_size
        self.project_id = project_id; self.is_cancelled = False
    def run(self):
        try:
            pil_image = process_single_gee_frame(self.aoi_coords, self.start1, self.end1, self.start2, self.end2,
                                                 self.threshold, self.thumb_size, self.project_id, self.signals.progress.emit)
            self.signals.finished.emit(pil_image)
        except Exception as e:
            tb_str = traceback.format_exc(); self.signals.error.emit(f"Error in GEEWorker: {e}\nTrace: {tb_str}")
...
class TimeLapseWorker(QObject):
    def __init__(self, aoi_coords, baseline_start_date, baseline_end_date, timelapse_start_year,
        ...
        super().__init__()
        self.signals = WorkerSignals(); self.aoi_coords = aoi_coords; self.baseline_start_date = baseline_start_date
        self.baseline_end_date = baseline_end_date; self.timelapse_start_year = timelapse_start_year
        self.timelapse_end_year = timelapse_end_year; self.threshold = threshold; self.thumb_size = thumb_size
        self.project_id = project_id; self.output_video_path = output_video_path; self.fps = fps; self.is_cancelled = False
    def run(self):
        temp_dir = tempfile.mkdtemp(prefix="galamsey_frames_")
        self.signals.progress.emit(f"Temp frames in: {temp_dir}")
        frame_paths = []; total_frames = self.timelapse_end_year - self.timelapse_start_year + 1
        try:
            for i, year in enumerate(range(self.timelapse_start_year, self.timelapse_end_year + 1)):
                ...
                try:
                    pil_image = process_single_gee_frame(self.aoi_coords, self.baseline_start_date, self.baseline_end_date,
                    ...
                    if pil_image:
                        ...
                        pil_image.save(frame_filename); frame_paths.append(frame_filename)
                        self.signals.progress.emit(f"Saved frame for {year}: {frame_filename}")
                    else: self.signals.progress.emit(f"Skipping frame for {year} (no image).")
                except Exception as frame_e: self.signals.progress.emit(f"Error for year {year}: {frame_e}. Skipping.")
            ...
            self.signals.progress.emit("Compiling video..."); first_frame_img = cv2.imread(frame_paths[0])
            if first_frame_img is None: self.signals.error.emit(f"Could not read first frame: {frame_paths[0]}"); return
            height, width, _ = first_frame_img.shape; size = (width, height)
            fourcc = cv2.VideoWriter_fourcc(*'mp4v'); out_video = cv2.VideoWriter(self.output_video_path, fourcc, self.fps, size)
            for frame_path in sorted(frame_paths):
                img = cv2.imread(frame_path)
                if img is not None: out_video.write(img)
                else: self.signals.progress.emit(f"Warning: Could not read {frame_path} for video.")
            out_video.release(); self.signals.progress.emit(f"Video complete: {self.output_video_path}")
            self.signals.finished.emit(self.output_video_path)
        except Exception as e: tb_str = traceback.format_exc(); self.signals.error.emit(f"Time-lapse error: {e}\nTrace: {tb_str}")
        finally:
            self.signals.progress.emit("Cleaning up temp frames...")
            for fp in frame_paths:
                try: os.remove(fp)
                except OSError: pass
            try: os.rmdir(temp_dir); self.signals.progress.emit("Temp directory removed.")
            except OSError: self.signals.progress.emit(f"Could not remove temp dir: {temp_dir}. Remove manually.")
...
class GalamseyMonitorApp(QWidget):
    def __init__(self):
        ...
        self.timelapse_worker_thread = None; self.timelapse_worker = None
        self.progress_dialog = None; self.project_id = 'galamsey-monitor'
        ...
        for dt_edit in [self.date1_start, self.date1_end, self.date2_start, self.date2_end]:
            dt_edit.setCalendarPopup(True); dt_edit.setDisplayFormat("yyyy-MM-dd")
        ...
        self.timelapse_start_year_input = QSpinBox(); self.timelapse_start_year_input.setRange(2000, QDate.currentDate().year()); self.timelapse_start_year_input.setValue(2018)
        self.timelapse_end_year_input = QSpinBox(); self.timelapse_end_year_input.setRange(2000, QDate.currentDate().year() + 5); self.timelapse_end_year_input.setValue(2022)
        self.timelapse_fps_input = QSpinBox(); self.timelapse_fps_input.setRange(1, 30); self.timelapse_fps_input.setValue(1)
        ...
        status_v_layout = QVBoxLayout(); self.status_log = QTextEdit(); self.status_log.setReadOnly(True); self.status_log.setFixedHeight(100)
        status_v_layout.addWidget(self.status_log); status_group.setLayout(status_v_layout); main_layout.addWidget(status_group)
    ...
    def init_gee_check(self):
        ...
        try: ee.Initialize(project=self.project_id); self.log_status("GEE initialized.")
        except Exception as e: self.handle_gee_init_error(e)
    def handle_gee_init_error(self, e):
        ...
        self.log_status(msg.replace("\n\n", "\n").replace("\n", "\nStatus: ")); QMessageBox.critical(self, "GEE Init Error", msg)
        self.analyze_button.setEnabled(False); self.generate_timelapse_button.setEnabled(False)
    def log_status(self, message): self.status_log.append(message); QApplication.processEvents()
    def setup_progress_dialog(self, title="Processing..."):
        if self.progress_dialog: self.progress_dialog.close()
        self.progress_dialog = QProgressDialog(title, "Cancel", 0, 100, self)
        self.progress_dialog.setWindowModality(Qt.WindowModality.WindowModal); self.progress_dialog.setAutoClose(False)
        self.progress_dialog.setAutoReset(False); self.progress_dialog.setMinimumDuration(0); self.progress_dialog.setValue(0)
        return self.progress_dialog

    # --- Single Analysis Methods ---
    def run_single_analysis(self):
        self.log_status("Starting single analysis..."); self.analyze_button.setEnabled(False); self.generate_timelapse_button.setEnabled(False)
        self.map_label.setText("Processing..."); self.map_label.setPixmap(QPixmap())
        try:
            ...
            if not (aoi_coords[0] < aoi_coords[2] and aoi_coords[1] < aoi_coords[3]): raise ValueError("Coords order issue.")
            start1 = self.date1_start.date().toString("yyyy-MM-dd"); end1 = self.date1_end.date().toString("yyyy-MM-dd")
            start2 = self.date2_start.date().toString("yyyy-MM-dd"); end2 = self.date2_end.date().toString("yyyy-MM-dd")
            threshold = self.threshold_input.value()
            if self.date1_start.date() >= self.date1_end.date() or \
               self.date2_start.date() >= self.date2_end.date() or \
               self.date1_end.date() >= self.date2_start.date(): raise ValueError("Date ranges invalid/overlapping.")
        except ValueError as ve:
            self.log_status(f"Input Error: {ve}"); QMessageBox.warning(self, "Input Error", f"{ve}")
            self.analyze_button.setEnabled(True); self.generate_timelapse_button.setEnabled(True); return
        progress_dialog = self.setup_progress_dialog("Single Analysis..."); progress_dialog.canceled.connect(self.cancel_single_analysis); progress_dialog.show()
        self.worker = GEEWorker(aoi_coords, start1, end1, start2, end2, threshold, project_id=self.project_id)
        self.worker_thread = threading.Thread(target=self.worker.run, daemon=True)
        self.worker.signals.finished.connect(self.on_single_analysis_complete); self.worker.signals.error.connect(self.on_single_analysis_error)
        self.worker.signals.progress.connect(self.update_progress_label_only); self.worker_thread.start()
    ...
    def on_single_analysis_complete(self, pil_image):
        ...
        if pil_image:
            try:
                img_byte_array = io.BytesIO(); pil_image.save(img_byte_array, format='PNG'); img_byte_array.seek(0)
            ...
            except Exception as e: self.log_status(f"Error display map: {e}"); self.map_label.setText("Error displaying map.")
        else: self.map_label.setText("No significant change or error.")
    ...
    def on_single_analysis_error(self, error_message):
        ...
        self.map_label.setText("Analysis failed."); self.analyze_button.setEnabled(True); self.generate_timelapse_button.setEnabled(True)
    ...
    def run_timelapse_generation(self):
        self.log_status("Starting time-lapse..."); self.analyze_button.setEnabled(False); self.generate_timelapse_button.setEnabled(False)
        try:
            coords_str = self.coord_input.text().strip().split(',');
            if len(coords_str) != 4: raise ValueError("Coords: 4 numbers.")
            aoi_coords = [float(c.strip()) for c in coords_str]
            if not (aoi_coords[0] < aoi_coords[2] and aoi_coords[1] < aoi_coords[3]): raise ValueError("Coords order issue.")
            baseline_start = self.date1_start.date().toString("yyyy-MM-dd"); baseline_end = self.date1_end.date().toString("yyyy-MM-dd")
            tl_start_year = self.timelapse_start_year_input.value(); tl_end_year = self.timelapse_end_year_input.value()
            fps = self.timelapse_fps_input.value(); threshold = self.threshold_input.value()
            ...
            if QDate(tl_start_year,1,1) <= self.date1_end.date(): raise ValueError("Timelapse start must be after baseline.")
        ...
        progress_dialog = self.setup_progress_dialog("Generating Time-Lapse..."); progress_dialog.setRange(0, tl_end_year - tl_start_year + 1)
        progress_dialog.canceled.connect(self.cancel_timelapse_generation); progress_dialog.show()
        self.timelapse_worker = TimeLapseWorker(aoi_coords, baseline_start, baseline_end, tl_start_year, tl_end_year,
                                                threshold, project_id=self.project_id, output_video_path=self.temp_video_path, fps=fps)
    ...
    def update_timelapse_progress_value(self, current_frame, total_frames):
        if self.progress_dialog and self.progress_dialog.isVisible():
            self.progress_dialog.setRange(0, total_frames); self.progress_dialog.setValue(current_frame)
